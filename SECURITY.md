...
## 🔁 재점검 — 관리자 기능 추가 후 (2026-07-01)

통계·사용자 관리·전체 댓글·사용자 삭제 함수를 추가한 뒤, 새 공격 표면을 다시 침투 테스트했습니다.

| 공격 시도 (anon 키) | 결과 |
|---|---|
| 비로그인 작품 삭제 | 🚫 HTTP 204지만 **실제 0건 삭제**(RLS 필터) — 데이터 그대로 확인 |
| 비로그인 댓글 삭제 | 🚫 0건 삭제 |
| 비로그인 `delete-user` 함수 호출 | 🚫 미배포 404 / 배포 후에도 서버가 `role='admin'` 검증 |
| 라이선스 전체 훔쳐읽기 | 🚫 빈 배열 |

> ⚠️ 참고(낮은 위험): `profiles` 는 누구나 읽을 수 있어 **username·role·id 가 노출**됩니다(이메일은 auth 에만 있어 안전). "누가 관리자인지"가 보이지만, role 변경은 차단되므로 악용은 어렵습니다. 더 조이려면 username 만 담은 공개 뷰를 따로 두는 방법이 있습니다(선택).

**안전망 추가**: `src/ErrorBoundary.jsx` — 화면이 예기치 못하게 멈춰도 흰 화면 대신 안내가 뜹니다. 문제 해결 지도는 `TROUBLESHOOTING.md` 참고.
